...
<div class="p-6 text-gray-900 dark:text-gray-100">
    ...
    @foreach ($this->contacts as $contact)
        <div class="flex gap-2 items-center justify-between w-full cursor-pointer p-3 bg-white dark:bg-gray-800">
            <div class="w-full">
                ...
                @if ($contact->totalRemaining > 0)
                    <div class=" font-extrabold text-green-600 dark:text-green-400">
                        {{ number_format($contact->totalRemaining, $this->selectedCountry->decimal_points) }}
                        <span class=" uppercase text-xs font-thin">{{ $this->selectedCountry->currency }}</span>
                    </div>
                @endif
            </div>
